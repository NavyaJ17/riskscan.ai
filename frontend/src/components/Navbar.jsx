...
function Navbar() {
  ...
  return (
    <div className="bg-gray-950 flex flex-row items-center justify-between fixed z-10 top-0 left-0 min-w-screen px-4 lg:px-8 py-2 outline outline-white/10">
      <Link to={"/"}>
        <div className="flex flex-row text-white items-center gap-2 text-xl font-bold font-outfit">
          <svg
            id="logo-71"
            width="43"
            height="48"
      ...
      {isLoggedIn ? (
        ...
        <div className="flex flex-row">
          <Link
            to={"/login"}
            className=" text-white text-sm font-semibold sm:py-2 sm:px-4 hover:underline hover:text-blue-500"
          >
            Login
          </Link>
          <Link to={"/signup"}>
            <button className="hidden sm:block bg-gray-700 text-white text-sm font-semibold py-2 px-4 rounded-full cursor-pointer hover:bg-gray-600  disabled:bg-gray-800 disabled:text-gray-300 disabled:cursor-not-allowed">
              Sign Up
            </button>
          </Link>
        </div>
